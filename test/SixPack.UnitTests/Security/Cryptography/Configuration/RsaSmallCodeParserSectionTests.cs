// RsaSmallCodeParserSectionTests.cs 
//
...
{
    [TestFixture]
    public class RsaSmallCodeParserSectionTests
    {
        [Test]
        public void ShouldLoad()
        {
            RsaSmallCodeParserSection config = (RsaSmallCodeParserSection)ConfigurationManager.GetSection("RsaSmallCodeParserSection_ShouldLoad");
            Assert.AreEqual("ABCDEFGHIJ", config.InitialAlphabet);
            Assert.AreEqual("0123456789", config.FinalAlphabet);
            Assert.AreEqual(3, config.CodeLength);
            Assert.AreEqual(1, config.PublicKey.Value.Modulus.IntValue());
            Assert.AreEqual(2, config.PublicKey.Value.Exponent.IntValue());
        }
