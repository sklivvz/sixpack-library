...
namespace SixPack.Text
{
	...
	public static class RegularExpressionPatterns
	{
		...
		/// </summary>
		public const string HexString = @"^([0-9a-fA-F]{2})*$";

		/// <summary>
		/// Matches a base-64 string.
		/// </summary>
		public const string Base64String = @"^([A-Za-z0-9+/]{4})*(([A-Za-z0-9+/]{3}=)|([A-Za-z0-9+/]{2}==)|([A-Za-z0-9+/]{1}===))?$";

		/// <summary>
		/// Matches a base-64 string, as encoded by <see cref="HttpServerUtility.UrlTokenEncode"/>.
		/// </summary>
		public const string UrlTokenString = @"^[A-Za-z0-9-_]*$";
	}
}