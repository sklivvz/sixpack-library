...
namespace SixPack.Caching
{
	internal static class PrefetchCacheController
	{
		...
		static PrefetchCacheController()
		{
#if DEBUG
			Console.WriteLine("[PrefetchCache] Starting.");
#endif
		...
		internal static TReturn GetFromCache<TReturn>(string cacheName, RefreshFunction<TReturn> del, int cacheTime, PrefetchCacheOptions prefetchCacheOptions)
		{
			...
			if ((prefetchCacheOptions & PrefetchCacheOptions.AllowExpired) != 0 ||
			    agent.Content.ExpiryDate >= DateTime.Now)
			{
#if DEBUG
